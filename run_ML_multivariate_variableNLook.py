...
# 49/50, 47
model = Sequential()
model.add(LSTM(lstm_units, input_shape=(train_X.shape[1], train_X.shape[2]), dropout=0.4))
model.add(Dense(numPCs))
...
fig, ax = plt.subplots()
ax.plot(xplot_shift,dataobs_shift_fullspan,'0.6',alpha=0.1)
...
ax.grid()
ax.set_xlabel('x [m]')
ax.set_ylabel('z, observed [m]')
ax.set_xlim(0,130)
fig, ax = plt.subplots()
ax.plot(xplot_shift,reconstruct_prof_fullspan,'tab:blue',alpha=0.1)
ymean = np.nanmean(reconstruct_prof_fullspan,axis=1)
ystd = np.nanstd(reconstruct_prof_fullspan,axis=1)
ax.plot(xplot_shift,ymean,'k')
ax.plot(xplot_shift,ymean+ystd,'k:')
ax.plot(xplot_shift,ymean-ystd,'k:')
ax.grid()
ax.set_xlabel('x [m]')
ax.set_ylabel('z, reconstructed [m]')
ax.set_xlim(0,130)
fig, ax = plt.subplots()
ax.plot(xplot_shift,dataobs_shift_fullspan - reconstruct_prof_fullspan,'tab:red',alpha=0.1)
ax.grid()
ax.set_xlabel('x [m]')
ax.set_ylabel('z [m]')
ax.set_xlim(0,130)
...
iiplot = np.arange(686,686 + Npred + Nlook)
iiplot = np.arange(10808,10808 + Npred + Nlook)

ax[0].plot(datahydro_fullspan[0,iiplot])
ax[1].plot(datahydro_fullspan[1,iiplot])
ax[2].plot(datahydro_fullspan[2,iiplot])
ax[3].plot(datahydro_fullspan[3,iiplot])