...
def run_hydrocollect_func(noaawlfloc, noaawlext, lidarhydrofloc, lidarhydroext, time_pickle_path):
...
def run_wavecollect17m_func(wave17mfloc, wave17mext,time_pickle_path):
...
def run_wavecollect8m_func(wave8mfloc, wave8mext,time_pickle_path):
...
def run_lidarwavegauge_func(lidarwavegaugefloc, lidarwavegaugeext,time_pickle_path):
...
def run_lidarrunup_func(lidarrunupfloc, lidarrunupext,time_pickle_path):
...
def run_wavecollect26m_func(wave26mfloc, wave26mext,time_pickle_path):
    ...
    for fname_ii in fname_in_range:
        ...
        wave26m_dir = np.append(wave26m_dir, wave_dir, axis=0)
    # Trim full data set to just the obs of interest
...
def run_getWIS_func(wisfloc, wisext,time_pickle_path):
    ...
    # Trim full data set to just the obs of interest
    ij_in_range = (timeWaveWIS >= epoch_beg) & (timeWaveWIS <= epoch_end)
    waveWIS_time = timeWaveWIS[ij_in_range]
    waveWIS_Tp = TpWIS[ij_in_range]
    waveWIS_Hs = HsWIS[ij_in_range]
    waveWIS_dir = DmWIS[ij_in_range]
    waveWIS_Tp[waveWIS_Tp < -99] = np.nan
    waveWIS_Hs[waveWIS_Hs < -99] = np.nan
    waveWIS_dir[waveWIS_dir < -99] = np.nan

    return waveWIS_time,waveWIS_Tp,waveWIS_Hs,waveWIS_dir
