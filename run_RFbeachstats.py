...
nummorph = 3
numoutput = 1
num_features = numhydro + nummorph
inputData = np.empty((1,num_features))*np.nan
outputData = np.empty((1,))*np.nan
ii_tplot = []
for tt in np.arange(25,time_fullspan.size-1):

    # get input hydro
    ds_watlev = hydro_fullspan[0,tt]
    ds_Hs = hydro_fullspan[1,tt]
    ds_Tp = hydro_fullspan[2,tt]
    ds_wdir = hydro_fullspan[3,tt]
    # get input morph conditions
    ds_beachstat = beachstat_fullspan[tt]
    ds_beachstatm6 = beachstat_fullspan[tt - 6]
    ds_beachstatm12 = beachstat_fullspan[tt - 12]
    ds_beachstatm24 = beachstat_fullspan[tt - 24]
    # check for nans....
    ds_data = np.column_stack((ds_watlev,ds_Hs,ds_Tp,ds_wdir,ds_beachstatm6,ds_beachstatm12,ds_beachstatm24,ds_beachstat))
...
feature_names = ['$\overline{\eta}$','$H_s$','$T_p$','$\\theta_p$','$X_{t-6}$','$X_{t-12}$','$X_{t-24}$']
importances = regressor.feature_importances_
std = np.std([tree.feature_importances_ for tree in regressor.estimators_], axis=0)
forest_importances = pd.Series(importances)
fig, ax = plt.subplots()
# forest_importances.plot.bar(yerr=std, ax=ax)
ax.bar(np.arange(7),importances)
...
for nn in np.arange(storm_timeend_all.size):
    ...
    for tt in np.arange(Npred):
        ...
        # get input hydro
        ds_watlev = hydro_fullspan[0, ttlook]
        ds_Hs = hydro_fullspan[1, ttlook]
        ds_Tp = hydro_fullspan[2, ttlook]
        ds_wdir = hydro_fullspan[3, ttlook]
        # get input morph conditions
        ds_beachstat = beachstat_fullspan[ttlook]
        if tt <= 24:
            ds_beachstatm6 = beachstat_fullspan[ttlook - 6]
            ds_beachstatm12 = beachstat_fullspan[ttlook - 12]
            ds_beachstatm24 = beachstat_fullspan[ttlook - 24]
        elif tt > 24:
            ds_beachstatm6 = prev_pred[tt - 6]
            ds_beachstatm12 = prev_pred[tt - 12]
            ds_beachstatm24 = prev_pred[tt - 24]
        # check for nans....
        ds_data = np.column_stack((ds_watlev, ds_Hs, ds_Tp, ds_wdir, ds_beachstatm6, ds_beachstatm12, ds_beachstatm24))
        if np.sum(np.isnan(ds_data)) == 0:
            input_newDS = np.empty((1, num_features)) * np.nan
    ...
    if sum(~np.isnan(prev_pred)) >= 20:
        ...
        if len(tplotpre) > 0:
            ...
            ax.plot(tplotpost, prev_pred, 'xm', linewidth=40)
            pred_smooth = np.convolve(prev_pred, np.ones(12) / 12, mode="valid")
